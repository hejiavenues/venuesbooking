(function($) {
    "use strict";
    //++++++++++
    $.fn.bootstrapTreeTable_sm = function(options, param) {
        // 如果是调用方法
        if (typeof options == 'string') {
            return $.fn.bootstrapTreeTable_sm.methods[options](this, param);
        }

        // 如果是初始化组件
        options = $.extend({}, $.fn.bootstrapTreeTable_sm.defaults, options || {});
        // 是否有radio或checkbox
        ...
        target.load = function(parms){
            ...
            var thead = $('<thead class="treegrid-thead" style="   height: 20px; font-size: 12px;"></thead>');
            thead.append(thr);
            target.append(thead);
            // 构造表体
            var tbody = $('<tbody class="treegrid-tbody" style="  font-size: 12px;" ></tbody>');
            target.append(tbody);
            // 添加加载loading
            ...
            $.ajax({
                ...
                success : function(data, textStatus, jqXHR) {
                    ...
                    target.find("tbody").find("tr").dblclick(function(){
                    	options.dblClick&&options.dblClick($(this));
                    });
                },
    ...
    // 组件方法封装........
    $.fn.bootstrapTreeTable_sm.methods = {
    ...
    $.fn.bootstrapTreeTable_sm.defaults = {
        id : 'menuId',// 选取记录返回的值
