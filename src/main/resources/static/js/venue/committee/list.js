...
var vm = new Vue({
	...
	data: {
		...
		table:{//表格数据
			  "col":[
					...
					{field : "cname", title : "名称", width : ""}, 
					{field : "connectMobile", title : "联系方式", width : ""}, 
					{field : "connectPerson", title : "负责人", width : ""}, 
					{field : "address", title : "地址", width : ""}, 
	...
	methods : {
		...
		save: function() {
			dialogOpen({
				title: '新增居委会',
				url: 'venue/committee/add.html?_' + $.now(),
				width: '30%',
				height: '60%',
		...
		edit: function(row) {
			...
			if(checkedRow(ck)){
				dialogOpen({
					title: '编辑居委会',
					url: 'venue/committee/edit.html?_' + $.now(),
					width: '30%',
					height: '60%',
					success: function(iframeId){
						top.frames[iframeId].vm.bCommittees.cid = ck[0].cid;
		...
		remove: function(row) {
			...
			if(checkedArray(ck)){
				...
				$.RemoveForm({
					url: '../../venuesbook/committee/remove?_' + $.now(),
			    	param: ids,
