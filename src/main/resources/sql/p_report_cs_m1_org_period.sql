...
-- 建临时表
     ...
     out_amount_sum int(20),
	  ...
	  select
		   ...
		   GROUP BY  inner_org, DATE_FORMAT(a.inner_time,v_type ),a.period ) a ;
		  ...
		  -- create temporary TABLE   tmp_cs_day_org_all as
			select * from (
			 ...
			 ORDER BY  if(org_name='总计','',org_name) desc,inner_org ,a.inner_time,period  ;
