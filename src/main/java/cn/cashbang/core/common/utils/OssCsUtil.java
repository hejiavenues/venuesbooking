 ...
 *
 */
public class OssCsUtil {
	
	private static final Logger logger = LoggerFactory.getLogger(OssCsUtil.class);
	 ...
	 * 文本
	 */
	public static String DISKIMAGENAME = EVN + SpringContextUtils.getApplicationProperties().getOss().get("csimg_DISKIMAGENAME");
	...
	public static final boolean  OssFileIsExists( String ossUrl) {
		...
		OSSClient client=OssCsUtil.getOSSClient();
		try {
    ...
    */  
     ...
     */  
     ...
     public static final void downloadFile(String key,String fileName) {
    	 OssCsUtil.getOSSClient().getObject(new GetObjectRequest(BUCKETNAME, DISKIMAGENAME+key), 
				 new File(fileName));
     }
     ...
     public static final void  downloadByBrowser( HttpServletResponse response,String fileName) {
    	 OSSClient	 ossClient= OssCsUtil.getOSSClient();
