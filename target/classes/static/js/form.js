		...
		$.ajax({
			...
			error : function(XMLHttpRequest, textStatus, errorThrown) {
                ...
                if(XMLHttpRequest.responseJSON.code == 401){
                    ...
                    dialogMsg(zsl("请求超时，请稍候重试..."), "error");
                } else {
                    dialogMsg(errorThrown, 'error');
				}
			},
			beforeSend : function() {
				dialogLoading(true,options.time);
			},
			complete : function() {
				dialogLoading(false);
			}
...
$.ConfirmForm = function(options,time) {
	var defaults = {
		msg : zsl("您确定要保存当前数据项修改操作吗？"),
		url : "",
		param : {},
		type : "post",
		dataType : "json",
